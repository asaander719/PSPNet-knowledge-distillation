...
class PSPNet(nn.Module):
    def __init__(self, tag='student', layers=18, bins=(1, 2, 3, 6), dropout=0.1, classes=21, zoom_factor=8, use_ppm=True, criterion=nn.CrossEntropyLoss(ignore_index=255), pretrained=True):
        ...
        elif self.training and self.tag=='teacher':
    ...
    def forward(self, x, y=None):
        ...
        if self.training and self.tag=='student':
            aux_s = self.aux_s(x_tmp)
            if self.zoom_factor != 1:
                aux_s = F.interpolate(aux_s, size=(h, w), mode='bilinear', align_corners=True)
            return x, aux_s
        else:
            return x

        # if self.training and self.tag=='teacher':
        #     aux = self.aux(x_tmp)
        #     if self.zoom_factor != 1:
...
def teacher_loader():
    teacher_net = PSPNet(tag='teacher', layers=50, bins=(1, 2, 3, 6), dropout=0.0, classes=21, zoom_factor=8, use_ppm=True, criterion=nn.CrossEntropyLoss(ignore_index=255), pretrained=True)
    #model_path = './initmodel/ResNet50_train_epoch_200.pth'
    model_path = 'exp/voc2012_50/pspnet50/model/train_epoch_28_50layers.pth'
    #model_path = 'train_epoch_50.pth'
    ...
    # del new_state_dict["aux.1.running_mean"]
    # del new_state_dict["aux.1.running_var"]
