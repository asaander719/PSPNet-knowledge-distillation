...
from tensorboardX import SummaryWriter  # 存储路径
...
def worker_init_fn(worker_id):#固定网络初始化
    random.seed(args.manual_seed + worker_id)


def main_process():#多分布式不用
    return not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % args.ngpus_per_node == 0)


def check(args):#改mask
...
def main():
    args = get_parser()#拿参数
    check(args)#check参数
    os.environ["CUDA_VISIBLE_DEVICES"] = ','.join(str(x) for x in args.train_gpu)#配环境
